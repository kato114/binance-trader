...
class BinanceAPI:
    ...
    BASE_FUTURE_URL = "https://fapi.binance.com/fapi/v1"
    #testnet
    #BASE_FUTURE_URL = "https://testnet.binancefuture.com/fapi/v1"
    ...
    def get_future_order_books(self, market, limit=50):
        path = "%s/depth" % self.BASE_FUTURE_URL
        params = {"symbol": market, "limit": limit}
        return self._get_no_sign(path, params)
    ...
    def get_future_exchange_info(self):
        path = "%s/exchangeInfo" % self.BASE_FUTURE_URL
        return requests.get(path, timeout=30, verify=True).json()
    ...
    def buy_future(self, market, quantity):
        path = "%s/order" % self.BASE_FUTURE_URL
        params = self._order(market, quantity, "BUY")
        return self._post(path, params, False)

    def sell_future(self, market, quantity):
        path = "%s/order" % self.BASE_FUTURE_URL
    ...
    def query_future_order(self, market, orderId):
        path = "%s/order" % self.BASE_FUTURE_URL
        params = {"symbol": market, "orderId": orderId}
        return self._get(path, params)
    ...
    def _sign(self, params={}, flag=True):
        data = params.copy()
    ...
    def _post(self, path, params={}, flag=True):
        params.update({"recvWindow": config.recv_window})
        query = urlencode(self._sign(params, flag))
        url = "%s" % (path)
        print("path: %s" % (url))
        print("params: %s" % (query))
        header = {"X-MBX-APIKEY": self.key}
