...
class Orders():
    ...
    def sell_limit(symbol, quantity, sell_price):
        ...
        return order
    @staticmethod
    def buy_future(symbol, quantity):

        order = client.buy_future(symbol, quantity)
    ...
    @staticmethod
    def sell_future(symbol, quantity):
        
        order = client.sell_future(symbol, quantity)
    ...
    @staticmethod
    def get_future_order_book(symbol):
        try:

            orders = client.get_future_order_books(symbol, 5)
            lastBid = float(orders['bids'][0][0]) #last buy price (bid)
            lastAsk = float(orders['asks'][0][0]) #last sell price (ask)
            ...
            print('get_future_order_book Exception: %s' % e)
            return 0, 0
    ...
    @staticmethod
    def get_future_order(symbol, orderId):
        try:

            order = client.query_future_order(symbol, orderId)
    ...
    def get_future_info(symbol):
        try:        
            ...
            print('get_future_info Exception: %s' % e)